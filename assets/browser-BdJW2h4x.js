...
const getProducts = http.get(`${BASE_URL$1}/products`, ({ request }) => {
  ...
  const { category = "전체", sort: sort2 } = params;
  const filteredAndSorted = MOCKING_PRODUCT_DATA.content.filter((product) => category === "전체" || product.category === category).sort((a, b) => sort2 === "price,desc" ? b.price - a.price : a.price - b.price);
  const responseData = {
...
const productHandler = [getProducts];
const content = [];
const pageable = {
  pageNumber: 0,
  pageSize: 20,
...
const cartItemHandler = [getCartItems, postCartItems, patchCartItems, deleteCartItems];
const handlers = [...productHandler, ...cartItemHandler];
